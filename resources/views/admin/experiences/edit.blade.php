...
<div class="max-w-3xl">
    ...
    <form action="{{ route('admin.experiences.update', $experience) }}" method="POST" class="bg-white rounded-lg shadow p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea name="description" rows="4" required
                      class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500">{{ old('description', $experience->description) }}</textarea>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="color" required class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500">
                <option value="blue" {{ old('color', $experience->color) == 'blue' ? 'selected' : '' }}>Blue</option>
                <option value="purple" {{ old('color', $experience->color) == 'purple' ? 'selected' : '' }}>Purple</option>
                <option value="green" {{ old('color', $experience->color) == 'green' ? 'selected' : '' }}>Green</option>
                <option value="red" {{ old('color', $experience->color) == 'red' ? 'selected' : '' }}>Red</option>
                <option value="yellow" {{ old('color', $experience->color) == 'yellow' ? 'selected' : '' }}>Yellow</option>
                <option value="indigo" {{ old('color', $experience->color) == 'indigo' ? 'selected' : '' }}>Indigo</option>
                <option value="pink" {{ old('color', $experience->color) == 'pink' ? 'selected' : '' }}>Pink</option>
            </select>
        </div>
