    ...
    <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow p-6">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" rows="4" required
                      class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description', $project->description) }}</textarea>
        </div>
        ...
        <div class="mb-4">
            ...
            @if($project->image)
                <div class="mb-2">
                    <img src="{{ asset($project->image) }}" alt="{{ $project->title }}" class="h-32 w-48 object-cover rounded">
                    <p class="text-sm text-gray-500 mt-1">Gambar saat ini</p>
                </div>
            @endif
            <input type="file" name="image" accept="image/*"
                   class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            <p class="text-sm text-gray-500 mt-1">Format: JPG, PNG, GIF. Max: 2MB. Kosongkan jika tidak ingin mengubah gambar.</p>
        </div>
        ...
        <div class="mb-4">
            <label class="flex items-center">
                <input type="checkbox" name="visible" value="1" {{ old('visible', $project->visible) ? 'checked' : '' }}
