...
@section('header', 'Tambah Certificate Baru')
...
<div class="max-w-3xl">
    ...
    <form action="{{ route('admin.certificates.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
        @csrf

        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-2">Nama Certificate *</label>
            <input type="text" name="name" value="{{ old('name') }}" required
                   placeholder="Contoh: Hitaclass – Frontend Web Developer"
                   class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500">
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-2">Tahun *</label>
            <input type="text" name="year" value="{{ old('year', date('Y')) }}" required maxlength="4"
                   class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500">
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-2">Level *</label>
            <input type="text" name="level" value="{{ old('level') }}" required
